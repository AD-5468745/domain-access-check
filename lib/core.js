...
export const STATUS = {
  ...
  // ★ 사이트는 살아 있는데 '사람 브라우저가 아니면' 막는 방화벽(Cloudflare 등)에 걸린 상태.
  //   한국에서 차단된 것과는 전혀 다르다 — 섞어 놓으면 거짓 경보가 된다(2026-09-05).
  blocked: { icon: '🛡', label: '봇차단' },
};
...
export function buildTelegramReport(results, { nowKst = '', round = '수동', skipped = [] } = {}) {
  ...
  } else if (c.up === c.total) {
    parts.push(quote([`총 ${c.total}개 모두 정상 ✅`]));
  } else {
    const sumLines = [
      `총 ${c.total}개`,
      `✅ 정상 ${c.up} · ⚠️ 제한 ${c.warn} · ❌ 이상 ${c.down} · 🔀 주소확인 ${c.redir}`,
    ];
    if (c.blocked) sumLines.push(`🛡 봇차단 ${c.blocked} — 사이트는 응답하지만 로봇 접속을 막습니다`);
