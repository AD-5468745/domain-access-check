...
// ── CSV 읽기 ─────────────────────────────────────────────────
/**
 * 구글시트 CSV 내려받기 결과 → 2차원 배열.
 * ★ 2026-09-10 에이든 결정 — 도메인 목록은 앱스스크립트를 거치지 않고
 *   시트 파일을 그대로 내려받아 읽는다(누드TV 리포트가 쓰는 방식과 같다).
 *   앱스스크립트 웹앱은 '임시 주소로 가서 받아라'는 왕복이 있어 한국 VPN 을
 *   지날 때 자주 어긋났다(실측 83%). CSV 는 그냥 파일 하나라 왕복이 없다.
 *
 * 규칙은 RFC4180 그대로 — 큰따옴표 안의 쉼표·줄바꿈·두 겹 따옴표를 지킨다.
 * 업체명에 쉼표가 들어가도 안전해야 하므로 직접 만든다(외부 의존성 0 유지).
 */
export function parseCsv(text) {
  const s = String(text == null ? '' : text).replace(/^﻿/, '');  // 엑셀이 붙이는 BOM 제거
  const rows = [];
  let row = [];
  let field = '';
  let quoted = false;
  let started = false;                       // 빈 문자열과 '값 없음'을 구분하기 위한 표시
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (quoted) {
      if (c === '"') {
        if (s[i + 1] === '"') { field += '"'; i++; }   // 따옴표 두 개 = 따옴표 한 글자
        else quoted = false;
      } else field += c;
      continue;
    }
  ...
  // 마지막 줄 — 끝에 줄바꿈이 있으면 빈 줄을 만들지 않는다
