...
export const STATUS = {
  ...
  //     그래서 '확인 필요' 목록에 넣지 않고, 정상으로 세되 표시만 따로 한다.
  blocked: { icon: '🛡', label: '봇차단(도달됨)' },
};
...
export function buildTelegramReport(results, { nowKst = '', round = '수동', skipped = [] } = {}) {
  ...
  const reached = c.up + c.blocked;          // 서버까지 닿은 것
  const botNote = c.blocked
    ? `🛡 그중 ${c.blocked}개는 사이트 방화벽이 로봇을 막은 화면 — 서버까지 도달은 확인됨`
    : '';

  if (c.total === 0) {
    parts.push(quote(['점검할 도메인이 없습니다.']));
  } else if (reached === c.total) {
    const okLines = [`총 ${c.total}개 모두 정상 ✅`];
    if (botNote) okLines.push(botNote);
    parts.push(quote(okLines));
  } else {
    const sumLines = [
      `총 ${c.total}개`,
      `✅ 정상 ${reached} · ⚠️ 제한 ${c.warn} · ❌ 이상 ${c.down} · 🔀 주소확인 ${c.redir}`,
    ];
    if (botNote) sumLines.push(botNote);
    parts.push(quote(sumLines));
    parts.push('⚠️ 확인 필요');
