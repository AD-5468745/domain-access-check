...
//  · normalizeDomain : 시트 셀 한 칸 → 점검 가능한 도메인(정규화) / 아니면 null
...
export const STATUS_ORDER = ['down', 'warn', 'redir', 'up'];
...
export function parseSheet(values) {
  ...
  for (let c = 0; c < maxCols; c++) {
    ...
    for (let r = 1; r < grid.length; r++) {
      ...
      seen.add(domain);
      domains.push({ company, domain, cell });
    }
  }
...
export function countByStatus(results) {
  const c = { total: 0, up: 0, warn: 0, down: 0, redir: 0 };
  for (const r of results || []) {
    c.total++;
