...
import { asciiHost, isIdnHost, judgeStatus, looksBotBlocked, looksKoreaBlocked, posNum } from './probe.js';
...
export async function recheckOne(ctx, entry, opts = {}) {
  ...
  try {
    ...
    // 한국 심의 차단이 최우선
    if (looksKoreaBlocked(finalUrl, body)) {
      return {
        status: 'down', http, finalUrl, redirectTo: hostOfUrl(finalUrl),
        note: '한국에서 차단됨(심의 차단 안내 페이지) · 브라우저로 확인',
      };
    }
    ...
    const hostAscii = asciiHost(host);
    const movedTo = (finalHost && hostAscii && !sameSite(finalHost, hostAscii)) ? finalHost : '';
    const judged = judgeStatus(http === null ? 200 : http);

    if (judged.status === 'up' && movedTo) {
      // ★ 한글주소는 넘어가는 것이 정상이다(에이든 지시 2026-09-06) — 1차 판정과 같은 규칙.
      if (isIdnHost(host)) {
        return { status: 'up', http, finalUrl, redirectTo: movedTo, note: '정상(한글주소 연결) · 브라우저로 확인' };
      }
